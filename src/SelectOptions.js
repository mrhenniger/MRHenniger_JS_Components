...
class SelectDropdown extends Components {
    ...
    __constructCore(core) {
        ...
        try {
            ...
            this.__core.find('option').each((opt) => {
                const d = new Dom(opt);
        ...
        let size = 'sd-medium';
        if (this.__core.hasClass('sd-small'))
            size = 'sd-small';
        else if (this.__core.hasClass('sd-large'))
            size = 'sd-large';
        this.__attributes = { size: size };
    }
    __constructDom() {
    ...
    __setTemplate() {
        ...
        this.__template = new Strings(`<select class='inner' {{ req }} {{ dis }}>${optionsHtml}</select>`);
    }
    __setHandlers() { }
}
